...
from backend.runtime import (
    ...
    _polish_product_answer,
    _product_slots_from_question,
    _resolve_generic_lookup,
)
from lstm_lookup import check_data_availability, normalize_whitespace, read_jsonl
...
def _is_correct(intent: str, prediction: str, gold: str) -> bool:
    ...
    if pred and pred in ref:
        return True
    return _token_f1(prediction, gold) >= 0.75 and _rouge_l(prediction, gold) >= 0.70
...
def _scan_product_coverage() -> Dict[str, object]:
    ...
    for switch_dir in sorted([path for path in root.iterdir() if path.is_dir()]):
        ...
        for version_dir in sorted([path for path in switch_dir.iterdir() if path.is_dir()]):
            ...
            if missing:
                missing_paths.append({"switch": switch, "version_dir": version_dir.name, "missing_files": missing})
...
def _predict_intents_batch(questions: Sequence[str], model, tokenizer, config: Dict[str, object], device: torch.device) -> List[str]:
    if not questions:
        return []
    max_length = int(config["max_length"])
    encoded = [tokenizer.encode(_clean(question), max_length) for question in questions]
    lengths = torch.tensor([len(ids) for ids in encoded], dtype=torch.long, device=device)
    max_tokens = max(len(ids) for ids in encoded)
    padded = [ids + [0] * (max_tokens - len(ids)) for ids in encoded]
    input_ids = torch.tensor(padded, dtype=torch.long, device=device)
    with torch.no_grad():
        logits = model(input_ids, lengths)
        predicted_ids = logits.argmax(dim=1).tolist()
    id_to_label = dict(config.get("id_to_label", {}))
    label_names = list(config.get("label_names", []))
    predictions: List[str] = []
    for predicted_id in predicted_ids:
        if not id_to_label:
            predictions.append(label_names[predicted_id])
            continue
        if str(predicted_id) in id_to_label:
            predictions.append(id_to_label[str(predicted_id)])
        elif predicted_id in id_to_label:
            predictions.append(id_to_label[predicted_id])
        else:
            predictions.append(id_to_label.get(str(predicted_id), label_names[predicted_id]))
    return predictions


def _run_domain_batched(runtime, rows: Sequence[Dict[str, object]], domain: str, batch_size: int) -> Tuple[List[Dict[str, object]], Dict[str, object]]:
    ...
    intents = _predict_intents_batch(questions, runtime.lstm_model, runtime.lstm_tokenizer, runtime.lstm_config, runtime.device)

    for index in range(0, len(rows), batch_size):
        batch_rows = rows[index:index + batch_size]
        batch_intents = intents[index:index + len(batch_rows)]
        for row, predicted_intent in zip(batch_rows, batch_intents):
            slots = dict(row["slots"])
            if domain == "product":
                slots = _product_slots_from_question(row["question"]) | slots
                availability_check = runtime._availability_check(slots)
                if not availability_check.get("available", True):
                    final_answer = "This particular data is not available in the current Aruba switch dataset."
                    ...
                    resolution = _resolve_generic_lookup(
                        ...
                        predicted_intent,
                        slots,
                        runtime.lookup_entries,
                        runtime.lookup_index,
                    )
                    ...
                    if lookup_status == "found" and lookup_answer:
                        final_answer = _polish_product_answer(lookup_answer, predicted_intent)
                        answer_source = "deterministic_lookup"
                    else:
                        final_answer = _format_deterministic("product", lookup_status)
                        answer_source = "lookup_fallback"
            else:
                availability_check = check_data_availability(slots, runtime.availability_index, runtime.bug_metadata_index)
                if not availability_check.get("available", True):
                    lookup_status = "data_not_available"
                    lookup_answer = ""
                    lookup_key_used = None
                    final_answer = _format_deterministic("release", lookup_status)
                    answer_source = "deterministic_availability"
                    confidence = 0.0
                    similarity = 0.0
                else:
            ...
            correct = _is_correct(predicted_intent, final_answer, gold_answer)
            lookup_correct = _is_correct(predicted_intent, lookup_answer or final_answer, gold_answer)

            if _is_exact_intent(predicted_intent):
                exact_total += 1
        ...
        print(f"  processed {min(index + batch_size, len(rows))}/{len(rows)} rows", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a fast 1000-question QA evaluation sweep.")
    ...
    per_domain_summary = {}
    coverage_report = {"product_docs": _scan_product_coverage()}

    print(f"device: {device}", flush=True)
    print(f"sample_size: {len(combined)}", flush=True)

    for domain in ("product", "release"):
        domain_rows = [row for row in combined if row["domain"] == domain]
        runtime = product_runtime if domain == "product" else release_runtime
        domain_results, domain_payload = _run_domain_batched(runtime, domain_rows, domain, args.batch_size)
        results.extend(domain_results)
        missing_data_rows.extend(domain_payload["missing_data_cases"])
        per_domain_summary[domain] = domain_payload["report"]
        print(f"completed {domain}: {len(domain_results)}", flush=True)
    ...
    with MISSING_JSON.open("w", encoding="utf-8") as handle:
        json.dump({"missing_data_cases": missing_data_rows, "coverage": coverage_report}, handle, indent=2, ensure_ascii=False)
